<div>
                ...
                <div class="bg-black">
                    <div class="relative">
                        <div class="mx-4 mt-8 py-1 sm:py-6">
                            <div id="slide-container-adidas" class="flex overflow-hidden">
                                <!-- Itens do Slide -->
                                @foreach ($sneakersNike as $sneaker)
                                    <div class="slide-item-adidas flex-shrink-0 w-1/1 md:w-1/4 mr-3 md:mr-2">
                                        <div
                                            ...
                                            <div class="mt-4 px-5 pb-5">
                                                <a href="/sneaker/{{ $sneaker->id }}/{{ $sneaker->slug }}"
                                                    wire:navigate>
                                                    <h5
                                                        class="text-xl tracking-tight text-slate-900 h-14 w-40 md:w-full">
                        ...
                        <div
                            ...
                            <button id="prev-btn-adidas" class="cursor-pointer rounded-full bg-black hover:bg-white">
                                <span
                        ...
                        <div
                            ...
                            <button id="next-btn-adidas" class="cursor-pointer rounded-full bg-black hover:bg-white">
                                <span
                                    class="inline-flex items-center justify-center w-10 h-9 group-focus:ring-4 group-focus:ring-white">
    ...
    <script>
        const slideContainerAdidas = document.getElementById("slide-container-adidas");
        const slideItemsAdidas = document.querySelectorAll(".slide-item-adidas");
        const numItemsAdidas = slideItemsAdidas.length;
        const itemsPerPageAdidas = 4;
        let currentPageAdidas = 0;

        function showPageAdidas(pageAdidas) {
            const startIndex = pageAdidas;
            const endIndex = startIndex + itemsPerPageAdidas;

            slideItemsAdidas.forEach((item, index) => {
                const adjustedIndex = (index + numItemsAdidas) % numItemsAdidas;
                item.style.display = adjustedIndex >= startIndex && adjustedIndex < endIndex ? "block" : "none";
            });
        }

        function nextPage() {
            currentPageAdidas = (currentPageAdidas + 1) % numItemsAdidas;
            showPageAdidas(currentPageAdidas);
        }

        function prevPage() {
            currentPageAdidas = (currentPageAdidas - 1 + numItemsAdidas) % numItemsAdidas;
            showPageAdidas(currentPageAdidas);
        }
        ...
        document.getElementById("next-btn-adidas").addEventListener("click", nextPage);
        document.getElementById("prev-btn-adidas").addEventListener("click", prevPage);
    </script>
</div>
